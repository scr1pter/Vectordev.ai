...
// The old Tauri app identifier changes between dev/beta/prod builds.
...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.vector.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `vector.settings.dat` is special: it maps to the `vector.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"vector.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // vector.settings.dat -> the electron settings store ("vector.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "vector.global.dat").
  const storeName = filename === "vector.settings.dat" ? "vector.settings" : filename
