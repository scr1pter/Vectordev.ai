...
export function createSpotlightSteps(host: SpotlightTourHost): SpotlightStep[] {
  ...
  return [
    ...
    workspace({
      ...
      body: "Open the project's files in full-screen tabs, edit by hand, or ask the same Vector agent to change the active file without leaving your task.",
      tip: "For a one-line tweak, editing here beats prompting.",
      target: "[data-vector-mode-switch] button:last-child",
      placement: "right",
    }),
    workspace({
      id: "tools-browser",
    ...
    workspace({
      ...
      body: "Send a report straight to the Vector team without leaving your workspace — your app version and platform ride along automatically. Software updates now live in full-screen Settings, so the workspace stays focused on the work.",
