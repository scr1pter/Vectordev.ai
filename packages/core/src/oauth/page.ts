...
// etc.).
//
// The visual language mirrors the Vector app so OAuth callback pages do not
// expose internal engine branding.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Vector is now connected to ${escapeHtml(provider)}.` : "Vector is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Vector couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Vector couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Vector.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Vector</title>
    <style>${STYLES}</style>
  </head>
  <body>
